...
/* Hooks a radio button group to the GnomePropertyBox */
static void
hook_radio_property (GtkWidget *dialog, GtkRadioButton *radio)
{
	GSList *group;
	GSList *l;
...
/* Hooks an option menu to the GnomePropertyBox */
static void
hook_option_menu_property (GtkWidget *dialog, GtkOptionMenu *omenu)
{
	if (GNOME_IS_PROPERTY_BOX (dialog)) {
		GtkMenu *menu;
		GList *l;
...
/* Hooks a spin button to the GnomePropertyBox */
static void
hook_spin_button_property (GtkWidget *dialog, GtkSpinButton *spin)
{
	GtkAdjustment *adj;
			    ...
			    gpointer value_var, gpointer info)
{
	...
	if (GTK_IS_RADIO_BUTTON (widget)) {
		...
		hook_radio_property (dialog, GTK_RADIO_BUTTON (widget));
	} else if (GTK_IS_OPTION_MENU (widget)) {
		hook_option_menu (dialog, GTK_OPTION_MENU (widget), value_var, info);
		hook_option_menu_property (dialog, GTK_OPTION_MENU (widget));
	} else if (GTK_IS_TOGGLE_BUTTON (widget)) {
		hook_toggle (dialog, GTK_TOGGLE_BUTTON (widget), value_var, info);
		hook_toggle_property (dialog, GTK_TOGGLE_BUTTON (widget));
	} else if (GTK_IS_SPIN_BUTTON (widget)) {
		hook_spin_button (dialog, GTK_SPIN_BUTTON (widget), value_var, info);
		hook_spin_button_property (dialog, GTK_SPIN_BUTTON (widget));
	} else if (GTK_IS_EDITABLE (widget)) {
		hook_editable (dialog, GTK_EDITABLE (widget), value_var, info);
		hook_editable_property (dialog, GTK_EDITABLE (widget));
	} else
		return FALSE;
...
/**
 * e_dialog_widget_hook_property:
 * @dialog: Dialog box in which the @widget lives  
 * @widget: A widget that will control a variable.
 *
 * Hooks a widget to a GnomePropertyBox.
 **/

gboolean
e_dialog_widget_hook_property (GtkWidget *dialog, GtkWidget *widget)
{
	g_return_val_if_fail (dialog != NULL, FALSE);
	g_return_val_if_fail (widget != NULL, FALSE);
	g_return_val_if_fail (GTK_IS_WIDGET (widget), FALSE);
