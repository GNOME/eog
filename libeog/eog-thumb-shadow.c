...
eog_thumb_shadow_add_round_border (GdkPixbuf **src)
{
	...
	dest = gdk_pixbuf_new (GDK_COLORSPACE_RGB,
			       TRUE, 8,
			       width + 2*ROUND_BORDER,
			       height+ 2*ROUND_BORDER);
	...
	gdk_pixbuf_copy_area (border,
			      ...
			      dest, 
			      0, height + ROUND_BORDER);
	/* upper right */
	gdk_pixbuf_copy_area (border,
			      ROUND_BORDER, 0, ROUND_BORDER, ROUND_BORDER,
			      dest, 
			      width + ROUND_BORDER, 0);
	/* down right */
	gdk_pixbuf_copy_area (border,
			      ROUND_BORDER, ROUND_BORDER, ROUND_BORDER, ROUND_BORDER,
			      dest, 
			      width + ROUND_BORDER, height + ROUND_BORDER);
