...
bm_compute_layout (GnomeWrapList *wlist, BMLayout *l)
{
	...
	{
		gint needed_space = l->n_items_minor * (l->item_minor + l->space_minor);
		if (l->size_minor - needed_space > l->item_minor*0.5)
			l->n_items_minor++;
	}
...
bm_update_scroll (GnomeWrapList *wlist)
{
	...
	priv->h_offset = priv->update_scroll_h_offset * size; /* update_scroll_*_offset are unit */
	priv->v_offset = priv->update_scroll_v_offset * size; /*  values! */
}

/* Updates the scroll adjustment values */
static void
bm_update_scroll_adjustments (GnomeWrapList *wlist)
{
	WrapListPrivate *priv;
	BlockModeInfo *bm;
	...
	if(data_len > bm->n_display) {
		adj->lower = 0.0;
		adj->upper = data_len/bm->items_per_block;
		if (data_len%bm->items_per_block > 0) adj->upper++;
		adj->step_increment = 1.0;
		adj->page_increment = bm->n_display/bm->items_per_block ;
	...
	for (i = update_first; i < update_last; i++, disp_i++)
		if (factory) {
			...
			if (!bm->items[disp_i]) {
				...
				gtk_signal_connect (GTK_OBJECT (bm->items [disp_i]),
						    "event",
			...
			gnome_list_item_factory_configure_item (
				...
				is_selected,
				is_focused);
			gtk_object_set_data (GTK_OBJECT (bm->items [disp_i]),
					     "IconNumber", 
					     GINT_TO_POINTER (i));

			/* Compute position */

			pos_minor = disp_i % bm->items_per_block;
			pos_major = disp_i / bm->items_per_block;
...
bm_update (GnomeWrapList *wlist)
{
	...
	if (display_last > data_len) display_last = data_len;
	...
	bm_update_scroll_adjustments (wlist);
